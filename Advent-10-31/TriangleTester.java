	...
	public static int countTrianglesA(String filename){
		try{
			...
			while(sc.hasNextLine()){
				...
				if(sc.hasNext()){nums1[0] = sc.next();}
				if(sc.hasNext()){nums1[1] = sc.next();}
				if(sc.hasNext()){nums1[2] = sc.next();}

				if(testTriangle(nums1)) c+=1;
			}
			sc.close();	
			return c;
	...
	public static int countTrianglesB(String filename){
		try{
			...
			while(sc.hasNextLine()){
				...
				if(sc.hasNext()){nums2[0] = sc.next();}
				if(sc.hasNext()){nums2[1] = sc.next();}
				if(sc.hasNext()){nums2[2] = sc.next();}
				
				if(sc.hasNext()){nums3[0] = sc.next();}
				if(sc.hasNext()){nums3[1] = sc.next();}
				if(sc.hasNext()){nums3[2] = sc.next();}
				//for(int i=0; i<3; i++){
